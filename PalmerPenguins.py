...
#1. Vamos a cargar el conjunto de datos. Muestra por pantalla el número de observaciones y 
#sus características. Mira el tipo de datos de cada una de sus columnas.
...
#2. Ya sabemos que este conjunto de datos tiene observaciones NA. 
#Vamos a eliminarlas y a verificar que efectivamente no queda ninguno:
...
print(penguins_rm.isna().sum()) #sumamos valores NA para comprobar si quedan.
...
#4. Vamos a añadir una columna, vamos a realizar una estimación (muy grosera) del área del pico de los pingüinos (bill) tal como si esta fuese un rectángulo.
#Esta nueva columnas se llama bill_area y debe encontrarse en la última posición. Verifica que es correcto.
...
print(penguins_rm.head()) #esta nueva columna ya está en la última columna.

#5. Hagamos algo un poco más elaborado, vamos a realizar una agrupación en función del sexo y de la especie de cada 
# observación donde obtendremos la longitud media del pico y solamente la referente al de las hembras.
